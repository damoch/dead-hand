...
namespace DeadHand.Scenarios.Implementations
{
    internal class FalseWarningScenario : ScenarioBase
    {
        ...
        {
            //first email - warning message (1-3 minute)
            _emails = new Dictionary<int, Email>
            {
                { _rng.Next(1,3) * 60 * 1000 ,
                    new Email()
            {
...
"
                ...
                {
                    _rng.Next(6, 8) * 60 * 1000, new Email()
            {
                Sender = "EMERGENCY ALERT SERVICE",
...
"
                ...
                {
                    _rng.Next(9, 12) * 60 * 1000, new Email()
            {
                Sender = "EMERGENCY ALERT SERVICE",
...
"
                ...
                {
                    _rng.Next(14, 16) * 60 * 1000, new Email()
            {
                Sender = "EMERGENCY ALERT SERVICE",
...
"
                ...
                {
                    _rng.Next(17, 18) * 60 * 1000, new Email()
            {
                Sender = "EMERGENCY ALERT SERVICE",
...
"
                ...
                {
                    _rng.Next(19, 21) * 60 * 1000,new Email()
            {
                Sender = "EMERGENCY ALERT SERVICE",
...
"
                ...
                {
                    _rng.Next(23, 25) * 60 * 1000, new Email()
            {
...
@"FA7S-I82B-HEY4-HWEF"
                ...
                {
                    _rng.Next(26, 26) * 60 * 1000, new Email()
            {
                Sender = "EMERGENCY ALERT SERVICE",
                Subject = "ALL CLEAR - THIS IS NOT A TEST",
                ReceivedDate = DateTime.Now,
...
$"Thanks for playing Dead Hand, developed by kszaku in October of 2020\n");
        ...
        public override void StartScenario()
        {
            ...
            {
                var newEvent = new Timer(email.Key);
                newEvent.Elapsed += SendEmail;
                newEvent.Start();
                _triggers.Add(newEvent);
            }
            return;
            var sendCivilDangerEmailEvent = new System.Timers.Timer(_rng.Next(1,3) * 60 * 1000); 
            ...
            var sendEvacuateImmediatlyEmailEvent = new System.Timers.Timer(_rng.Next(6, 8) * 60 * 1000); 
            ...
            var sendAirAttackWarning = new System.Timers.Timer(_rng.Next(9, 12) * 60 * 1000); 
            sendAirAttackWarning.Elapsed += SendAirAttackWarning;
            sendAirAttackWarning.Start();
            _triggers.Add(sendAirAttackWarning);

            var sendAllClearAirAttackMessage = new System.Timers.Timer(_rng.Next(14, 16) * 60 * 1000); 
            ...
            var sendCivilDanger2Message = new System.Timers.Timer(_rng.Next(17, 18) * 60 * 1000); 
            ...
            var sendPresidentialAddress = new System.Timers.Timer(_rng.Next(19, 21) * 60 * 1000); 
